...
while(bandera):
    try:
        calle=20#int(input("ingrese la cantidad  de calles: "))
        bandera=False
    except:
...
while(bandera):
    try:
        carrera=20#int(input("ingrese la cantidad  de carrera: "))
        bandera=False
    except:
        print("Error, debe ingresar un número entero")
bandera=True
while(bandera):
    try:
        habitantes=100#int(input("ingrese la cantidad de habitantes: "))
        bandera=False
    except:
        print("Error, debe ingresar un número entero")
bandera=True
while(bandera):
    try:
        numVehi=19#int(input("ingrese la cantitidad de vehículos: "))
...
def generarCalles(call):
    ...
    for i in range(0,call):
        ...
        if(ale==1 and (i-ult>=10)):
            if(ale==0):
                if(i%2==0):
                    calles.append([tipoVias[ale],i,"Oriente"])
                else:
                    calles.append([tipoVias[ale],i,"Occidente"])
            else:
                ult=i
                calles.append([tipoVias[ale],i,"Oriente-Occidente"])
        else:
            if(i%2==0):
                calles.append([tipoVias[0],i,"Oriente"])
            else:
                calles.append([tipoVias[0],i,"Occidente"])
    return calles
def generarCarreras(carr):
    ...
    for i in range(0,carr):
        ...
        if(ale==1 and (i-ult>=10)):
            if(ale==0):
                if(i%2==0):
                    carreras.append([tipoVias[ale],i,"Norte"])
                else:
                    carreras.append([tipoVias[ale],i,"Sur"])
            else:
                ult=i
                carreras.append([tipoVias[ale],i,"Norte-Sur"])   
        else:
            if(i%2==0):
                carreras.append([tipoVias[0],i,"Norte"])
            else:
                carreras.append([tipoVias[0],i,"Sur"])
    return carreras
    ...
    for i in range(0,len(calles)):
        for k in range(0,len(carreras)):
            ...
            if(i-ult[0]>=5 and ale==1):
                semaforos.append(["Calle",i,"Carrera",k,est[ale2]])
                print(i)
                ult=[i,k]
            elif(k-ult[1]>=5 and ale==1):
                semaforos.append(["Carrera",k,"Calle",i,est[ale2]])
                ult=[i,k]
            else:
                pass
    return semaforos               
...
def malladoVial(request):
    ...
    mallaVial=Via(vectorCalles,vectorCarreras)
    vectorSemaforos=generarSemaforos(mallaVial)
    return render(request,"index.html",{"carreras":vectorCarreras,"calles":vectorCalles,"semaforos":vectorSemaforos})

def inicio(request):
    return render(request,"formInicio.html")